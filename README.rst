...
`MIT-style permissive license <https://github.com/dshng/dirtt/raw/master/LICENSE.txt>`_.
...
``dirtt`` doesn't solve that problem directly. It provides a command-line
...
The basic usage is::
    ...
    $ dirtt -t TEMPLATE.xml 

The templates that come with ``dirtt`` will not work out of the box as they
are templates themselves. They are intended to be parsed by some other method 
to replace the inherent template variables that create a custom structure.
Alternatively these can be easily edited to create working templates.
